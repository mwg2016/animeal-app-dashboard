import React, { useEffect, useState } from 'react';
import axios from 'axios';
import { FaSearch } from 'react-icons/fa';

const Orders = () => {
  const [orders, setOrders] = useState([]);
  const [filteredOrders, setFilteredOrders] = useState([]);
  const [searchTerm, setSearchTerm] = useState('');
  const [loading, setLoading] = useState(true);
  const ordersdetails = import.meta.env.VITE_GET_ORDERS;

  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get(ordersdetails);
        setOrders(response.data.data);
        setFilteredOrders(response.data.data);
      } catch (error) {
        console.error('Error fetching orders:', error);
      } finally {
        setLoading(false);
      }
    };

    fetchOrders();
  }, []);

  const handleSearch = (e) => {
    const term = e.target.value.toLowerCase();
    setSearchTerm(term);
    const filtered = orders.filter((item) => {
      const createdAt = new Date(item.created_at).toLocaleDateString().toLowerCase();
      return (
        item.client_name.toLowerCase().includes(term) ||
        item.order_id.toString().includes(term) ||
        item.doctor_id.toString().includes(term) ||
        createdAt.includes(term)
      );
    });
    setFilteredOrders(filtered);
  };

  if (loading) {
    return (
      <div className="animate-pulse py-4 text-center pt-20">Loading Orders...</div>
    );
  }

  return (
    <div className="flex flex-col min-h-screen bg-gray-50 px-4 py-4 md:ml-64 pt-20">

      {/* Heading + Search */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 max-w-auto w-full mx-auto gap-4">
        <h2 className="text-2xl font-semibold text-[#B72765]">Orders Details</h2>
        <div className="w-full md:max-w-md border border-gray-300 rounded bg-white shadow-sm">
          <div className="flex items-center px-3 py-2 gap-2">
            <FaSearch className="text-gray-500 h-4 w-4" />
            <input
              type="text"
              value={searchTerm}
              onChange={handleSearch}
              placeholder="Search by name, Vet ID, Order ID, Date..."
              className="flex-1 rounded bg-white focus:outline-none placeholder-gray-500 text-sm"
            />
          </div>
        </div>
      </div>

      {/* No Orders Found */}
      {!loading && filteredOrders.length === 0 ? (
        <div className="text-center text-gray-500 text-lg mt-10">No Orders Found</div>
      ) : (
        <>
          {/* Desktop Table View */}
          <div className="hidden md:block w-full max-w-auto text-center bg-white shadow-lg rounded-lg mx-auto">
            <div className="bg-[#B72765] grid grid-cols-6 font-semibold text-sm px-6 py-4 text-white rounded-t-lg">
              <div>Order ID</div>
              <div>Client Name</div>
              <div>Vet's Id</div>
              <div>Total Payment</div>
              <div>Payment Status</div>
              <div>Date</div>
            </div>

            <div className="max-h-auto overflow-y-auto divide-y">
              {filteredOrders.map((item) => (
                <div
                  key={item.order_id}
                  className="grid grid-cols-6 px-6 py-4 text-sm text-gray-700 hover:bg-gray-50 transition"
                >
                  <div className="truncate">{item.order_id}</div>
                  <div className="truncate">{item.client_name}</div>
                  <div className="truncate">{item.doctor_id}</div>
                  <div className="truncate">{item.order_total}</div>
                  <div className="truncate">{item.payment_status}</div>
                  <div className="truncate">
                    {new Date(item.created_at).toLocaleString('en-US', {
                      year: 'numeric',
                      month: 'short',
                      day: '2-digit',
                    })}
                  </div>
                </div>
              ))}
            </div>
          </div>

          {/* Mobile Card View */}
          <div className="md:hidden w-full max-w-6xl mx-auto">
            {filteredOrders.map((item) => (
              <div
                key={item.order_id}
                className="mb-4 bg-white shadow-md p-4 rounded-lg hover:bg-gray-50 transition"
              >
                <h3 className="text-lg font-semibold text-[#B72765] mb-2">Order ID: {item.order_id}</h3>
                <div className="text-sm text-gray-700 space-y-1">
                  <div><strong>Client:</strong> {item.client_name}</div>
                  <div><strong>Vet's ID:</strong> {item.doctor_id}</div>
                  <div><strong>Total Payment:</strong> {item.order_total}</div>
                  <div><strong>Payment Status:</strong> {item.payment_status}</div>
                  <div><strong>Date:</strong> {new Date(item.created_at).toLocaleString('en-US', {
                    year: 'numeric',
                    month: 'short',
                    day: '2-digit',
                  }
                  )}</div>
                </div>
              </div>
            ))}
          </div>
        </>
      )}
    </div>
  );
};

export default Orders;
